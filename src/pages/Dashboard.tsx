...
import { Package, BookOpen, ShoppingCart, Clock, Sparkles, AlertTriangle } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-secondary-600">Welcome back! Here's what's happening in your kitchen.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-secondary-600">Pantry Items</p>
                <p className="text-2xl font-bold text-secondary-900">{stats.totalIngredients}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-secondary-600">Can Cook</p>
                <p className="text-2xl font-bold text-secondary-900">{stats.canCookRecipes}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-secondary-600">Total Recipes</p>
                <p className="text-2xl font-bold text-secondary-900">{stats.availableRecipes}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <Clock className={`h-8 w-8 ${stats.expiringSoon > 0 ? 'text-orange-600' : 'text-primary-600'}`} />
              </div>
              <div className="ml-4">
                <p className="text-sm font-medium text-secondary-600">Expiring Soon</p>
                <p className="text-2xl font-bold text-secondary-900">{stats.expiringSoon}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            {canCookRecipes.length > 0 ? (
              <div className="space-y-3">
                {canCookRecipes.map((recipe) => (
                  <div key={recipe.id} className="flex items-center space-x-3 p-3 bg-secondary-50 rounded-lg">
                    <img
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-secondary-900">{recipe.title}</h4>
              ...
              <div className="text-center py-6">
                ...
                <p className="text-secondary-600 mb-4">
                  Add ingredients to your pantry to discover recipes you can cook!
                </p>
                <Link to="/pantry">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center space-x-2">
              <AlertTriangle className={`h-5 w-5 ${stats.expiringSoon > 0 ? 'text-orange-600' : 'text-secondary-400'}`} />
          ...
          <CardContent>
            {expiringSoonItems.length > 0 ? (
              <div className="space-y-3">
                    ...
                    <div key={item.id} className="flex items-center justify-between p-3 bg-orange-50 rounded-lg border border-orange-200">
                      ...
                      <div className="text-right">
                        <span className={`text-sm font-medium ${
                          ...
                          daysLeft <= 3 ? 'text-orange-600' : 'text-orange-600'
                        }`}>
                ...
                <Link to="/pantry">
                  <Button variant="outline" className="w-full">Manage Pantry</Button>
                </Link>
              </div>
            ) : (
              <div className="text-center py-6">
        ...
        <Link to="/pantry">
          <Card className="hover:shadow-md transition-shadow cursor-pointer">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-sm text-secondary-600">Add and track ingredients</p>
            </CardContent>
          </Card>
        </Link>
        ...
        <Link to="/shopping">
          <Card className="hover:shadow-md transition-shadow cursor-pointer">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-sm text-secondary-600">Plan your grocery trips</p>
            </CardContent>
          </Card>
        </Link>
      </div>
