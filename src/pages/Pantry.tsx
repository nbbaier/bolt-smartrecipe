...
const CATEGORIES = [
  'Vegetables', 'Fruits', 'Meat', 'Dairy', 'Grains', 'Spices', 'Condiments', 'Other'
]

const UNITS = [
  'g', 'kg', 'ml', 'l', 'cups', 'tbsp', 'tsp', 'pieces', 'cans', 'bottles'
]
...
export function Pantry() {
  ...
  const categoryCounts = CATEGORIES.reduce((acc, category) => {
    acc[category] = ingredients.filter(ing => ing.category === category).length
    return acc
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-secondary-600">Track your ingredients and expiration dates</p>
        </div>
      ...
      {showAddForm && (
        <Card>
          <CardHeader>
            <CardTitle>{editingIngredient ? 'Edit Ingredient' : 'Add New Ingredient'}</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {filteredIngredients.length === 0 ? (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-secondary-900 mb-2">No ingredients found</h3>
            ...
            {!searchTerm && selectedCategory === 'All' && (
              <Button onClick={() => setShowAddForm(true)}>Add Your First Ingredient</Button>
            )}
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredIngredients.map((ingredient) => (
            <Card key={ingredient.id} className={`relative ${
              isExpired(ingredient.expiration_date) ? 'border-red-200 bg-red-50' :
              isExpiringSoon(ingredient.expiration_date) ? 'border-orange-200 bg-orange-50' : ''
            }`}>
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <h3 className="font-medium text-secondary-900">{ingredient.name}</h3>
                ...
                {ingredient.expiration_date && (
                  <div className={`flex items-center space-x-1 text-sm ${
                    isExpired(ingredient.expiration_date) ? 'text-red-600' :
                    isExpiringSoon(ingredient.expiration_date) ? 'text-orange-600' : 'text-secondary-600'
                  }`}>
                    {(isExpired(ingredient.expiration_date) || isExpiringSoon(ingredient.expiration_date)) && (
                ...
                {ingredient.notes && (
                  <p className="text-xs text-secondary-500 mt-2">{ingredient.notes}</p>
                )}
              </CardContent>
            </Card>
          ))}
