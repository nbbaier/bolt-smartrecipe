...
import { Calendar, Clock, ChefHat, Edit3, Trash2, AlertTriangle } from "lucide-react";
...
export function LeftoverCard({ leftover, onEdit, onDelete, className }: LeftoverCardProps) {
    ...
    <Card className={`relative ${
      ...
      <CardContent className="p-3 sm:p-4">
        ...
        {leftover.expiration_date && (
          <div className={`flex items-center space-x-1 text-xs sm:text-sm mb-2 ${getExpirationColor()}`}>
        ...
        <div className="flex items-center space-x-1 text-xs text-secondary-400 mt-2">
          ...
          <span>Added {new Date(leftover.created_at).toLocaleDateString()}</span>
        </div>
      </CardContent>
    </Card>
  );
