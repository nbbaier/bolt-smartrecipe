...
export function SmartCategorySelector({
  ...
  const analyzeCategory = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/categorize-ingredient`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="space-y-3">
      ...
      {showSuggestion && suggestion && suggestion.category !== currentCategory && (
      ...
      <select
        ...
        disabled={disabled}
        className="w-full h-10 rounded-lg border border-secondary-300 px-3 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
      >
